Experimentation with a language. Inspired by the minecraft mode Psi. Minimal pure functional language. Is supposed to be represented graphically, but for now exists as an (informal) subset of Python. Most language operations implemented as Lang_xxx() functions in main.py. The exception to this is Lang_choice_comparison - this operation is supposed to have lazy evaluation, and so is supposed to be implemented as an if statement calling Lang_choice_comparison for evaluation. test.py contains small examples of this code, with print statements serving as points of entry to the language.
...
- Each function is represented both with it's declaration and it's calls. When a function is declared, it is a square of tiles. When a function is called, it is a singular tile. All functions are in global scope. This implies recursion and cross-recursion.
